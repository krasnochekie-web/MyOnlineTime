...
private void handleNotificationIntent(Intent intent) {
    ...
    public void enforceLoginOverlays() {
        ...
        // === ПРОВЕРКА ПОДТВЕРЖДЕНИЯ НИКНЕЙМА ===
        boolean needsNicknameSetup = false;
        if (!noAuth) {
            if (!prefs.contains("is_nickname_confirmed")) {
                // Если это старый юзер (уже есть дата регистрации), автоматически пропускаем его
                if (prefs.contains("my_created_at") && !prefs.getString("my_created_at", "").isEmpty()) {
                    prefs.edit().putBoolean("is_nickname_confirmed", true).apply();
                } else {
                    needsNicknameSetup = true;
                }
            } else {
                needsNicknameSetup = !prefs.getBoolean("is_nickname_confirmed", false);
            }
        }
        // ========================================
        ...
        // Удаляем старые глобальные оверлеи (если есть)
        ...
        for (Fragment f : getSupportFragmentManager().getFragments()) {
            if (f != null && f.getView() instanceof ViewGroup) {
                ...
                if (needsAuthScreens) {
                    ...
                    if (noAuth) {
                        // 1. ПОЛЬЗОВАТЕЛЬ НЕ АВТОРИЗОВАН (ПОКАЗЫВАЕМ ВХОД)
                        ...
                        // 2. АВТОРИЗОВАН, НО НЕТ НИКНЕЙМА (ПОКАЗЫВАЕМ ЭКРАН НАСТРОЙКИ)
                        ...
                        // 3. ВСЕ ОТЛИЧНО (СКРЫВАЕМ ОБА ЭКРАНА)
                        if (loginOverlay != null) loginOverlay.setVisibility(View.GONE);
                        if (setupOverlay != null) setupOverlay.setVisibility(View.GONE);
                    }
                }
